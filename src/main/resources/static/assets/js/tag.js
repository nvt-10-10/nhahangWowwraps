$(document).ready(function () {
    ...
    $("#addForm").on('submit', function (event) {
        ...
        if (keyword != null && keyword.length > 0) {
            url = 'api/tag/search/' + keyword + '/';
        } else {
            url = 'api/tag/indexPage/';
        }
...
`;
    ...
    $("#List").on('click', '.edit-button', function () {
        ...
        $("#id").val(categoryid);
        var url = 'api/tag/Byid/' + categoryid;
        $.ajax({
    ...
    $("#confirm__Action__delete").on('click', function () {
        ...
        $.ajax({
            type: 'delete',
            url: "tag/delete?id=" + categoryid + "&keyword=" + keyword,
            success(reponse) {
                var url;
                ...
                if (reponse.error) {
                    ...
                    if (keyword != null && keyword.length > 0) {
                        url = 'api/tag/search/' + keyword + '/' + indexpage;
                    } else {
                        url = '/api/tag/indexPage/' + indexpage;
                    }
                    console.log("indexPage" + indexPage);
    ...
    $("#searchForm").on('submit', function () {
        ...
        $.ajax({
            ...
            type: 'get', url: 'tag/search', data: {keyword: keyword}, success(response) {
                $("#indexPage").val(0);
                if (keyword != null && keyword.trim().length > 0) {
                    updateList('api/tag/search/' + keyword + '/0');
                } else {
                    updateList('api/tag/indexPage/' + 0);
                }
